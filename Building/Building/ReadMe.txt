================================================================================
    MICROSOFT 基础类库 : Building 项目概述
===============================================================================

应用程序向导已为您创建了此 Building 应用程序。此应用程序不仅演示 Microsoft 基础类的基本使用方法，还可作为您编写应用程序的起点。

本文件概要介绍组成 Building 应用程序的每个文件的内容。
...
Building.h
    这是应用程序的主头文件。
    其中包括其他项目特定的标头（包括 Resource.h），并声明 CBuildingApp 应用程序类。

Building.cpp
    这是包含应用程序类 CBuildingApp 的主应用程序源文件。

Building.rc
    这是程序使用的所有 Microsoft Windows 资源的列表。它包括 RES 子目录中存储的图标、位图和光标。此文件可以直接在 Microsoft Visual C++ 中进行编辑。项目资源包含在 2052 中。

res\Building.ico
    这是用作应用程序图标的图标文件。此图标包括在主资源文件 Building.rc 中。
...
res\Toolbar.bmp
    此位图文件用于为工具栏创建平铺图像。
    初始工具栏和状态栏在 CMainFrame 类中构造。使用资源编辑器编辑此工具栏位图，并更新 Building.rc 中的 IDR_MAINFRAME TOOLBAR 数组以添加工具栏按钮。
/////////////////////////////////////////////////////////////////////////////

应用程序向导创建一个视图：

BuildingView.h、BuildingView.cpp - 文档视图
    这些文件包含 CBuildingView 类。
...
BuildingSet.h, BuildingSet.cpp
    这些文件包含 CBuildingSet 类。此类用于访问在向导中选定的数据源。
    不会添加任何序列化支持。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 Building.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
